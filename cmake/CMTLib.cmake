...
#---------------------------------------------------------
function(cmt_show_tests)
  message("cmt_show_tests>")
  get_property(tests GLOBAL PROPERTY CMT_TESTS)
  foreach(exe IN LISTS tests)
    get_property(project GLOBAL PROPERTY "CMT_${exe}_PROJECT")
    get_property(package GLOBAL PROPERTY "CMT_${exe}_PACKAGE")
    message ("Test ${exe} in in package ${package} of project ${project}")
  endforeach()
endfunction()

#---------------------------------------------------------
function(cmt_show_test exe)
  message("cmt_show_test> ${exe}")
  get_property(tests GLOBAL PROPERTY CMT_TESTS)
  list(FIND tests "${exe}" index)
  if("${index}" EQUAL "-1")
    message("Test ${exe} not found")
  else()
    get_property(project GLOBAL PROPERTY "CMT_${exe}_PROJECT")
    get_property(package GLOBAL PROPERTY "CMT_${exe}_PACKAGE")
    message ("Test ${exe} in package ${package} of project ${project}")
  endif()
endfunction()
...
function(cmt_action)
  ...
  elseif("${action}" STREQUAL "show tests")
    cmt_show_tests()
  elseif("${action}" MATCHES "show test ")
    string(REPLACE "show test " "" name "${action}")
    cmt_show_test(${name})
